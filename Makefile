...
vet: ## Run `go vet` in Docker
	docker build --target vet -f $(CURDIR)/Dockerfile.distroless -t toozej/go-listen:latest . 

test: ## Run `go test` in Docker
	docker build --progress=plain --target test -f $(CURDIR)/Dockerfile.distroless -t toozej/go-listen:latest . 

build: ## Build Docker image, including running tests
	docker build -f $(CURDIR)/Dockerfile.distroless -t toozej/go-listen:latest .
...
verify: get-cosign-pub-key ## Verify Docker image with Cosign
	cosign verify --key $(CURDIR)/go-listen.pub toozej/go-listen:latest

run: ## Run built Docker image
	docker run --rm --name go-listen --env-file $(CURDIR)/.env -p 8080:8080 toozej/go-listen:latest
...
install: ## Install go-listen from latest GitHub release
	if command -v go; then \
			go install github.com/toozej/go-listen@latest ; \
	else \
			echo "Downloading go-listen binary for $(OS)-$(ARCH)..."; \
			mkdir -p $(CURDIR)/tmp; \
			curl --silent -L -o $(CURDIR)/tmp/go-listen.tgz $(LATEST_RELEASE_URL); \
			tar -xzf $(CURDIR)/tmp/go-listen.tgz -C $(CURDIR)/tmp/; \
			chmod +x $(CURDIR)/tmp/go-listen; \
			sudo mv $(CURDIR)/tmp/go-listen /usr/local/bin/go-listen; \
			rm -rf $(CURDIR)/tmp; \
	fi
...
local-install: local-build local-verify ## Install compiled binary to local machine
	...
	sudo chmod 0755 /usr/local/bin/go-listen

upload-secrets-to-gh: ## Upload secrets from .env file to GitHub Actions Secrets + Dependabot
	$(CURDIR)/scripts/upload_secrets_to_github.sh go-listen 

upload-secrets-envfile-to-1pass: ## Upload secrets and .env file to 1Password
	$(CURDIR)/scripts/upload_secrets_to_1password secrets go-listen
	$(CURDIR)/scripts/upload_secrets_to_1password envfile go-listen
...
pre-commit-run: ## Run pre-commit hooks against all files
	...
	go-licenses report github.com/toozej/go-listen/cmd/go-listen
	govulncheck ./...
...
docs-generate:
	docker build -f $(CURDIR)/Dockerfile.docs -t toozej/go-listen:docs . 
	docker run --rm --name go-listen-docs -v $(CURDIR):/package -v $(CURDIR)/docs:/docs toozej/go-listen:docs

docs-serve: ## Serve documentation on http://localhost:9000
	docker run -d --rm --name go-listen-docs-serve -p 9000:3080 -v $(CURDIR)/docs:/data thomsch98/markserv
	$(OPENER) http://localhost:9000/docs.md
	@echo -e "to stop docs container, run:\n"
	@echo "docker kill go-listen-docs-serve"
