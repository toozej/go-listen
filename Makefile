...
REPO_URL := https://github.com/toozej/go-listen

# Detect the OS and architecture
OS := $(shell uname -s)
ARCH := $(shell uname -m)
LATEST_RELEASE_URL := $(REPO_URL)/releases/latest/download/go-listen_$(OS)_$(ARCH).tar.gz
...
vet: ## Run `go vet` in Docker
	docker build --target vet -f $(CURDIR)/Dockerfile -t toozej/go-listen:latest . 

test: ## Run `go test` with race detection in Docker
	docker build --progress=plain --target test -f $(CURDIR)/Dockerfile -t toozej/go-listen:latest . 

build: ## Build Docker image, including running tests
	docker build -f $(CURDIR)/Dockerfile -t toozej/go-listen:latest .
...
profile-cpu: ## Generate CPU performance profile
	...
	go test -bench=. -cpuprofile=$(CURDIR)/profiles/cpu.prof $(CURDIR)/internal/server/
...
profile-mem: ## Generate memory performance profile
	...
	mkdir -p $(CURDIR)/profiles
	go test -bench=. -memprofile=$(CURDIR)/profiles/mem.prof $(CURDIR)/internal/server/
